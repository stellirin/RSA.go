...
func TestRSAKeyPair(t *testing.T) {
	...
	SetMaxDigits(262)
	key := NewRSAKeyPair("deadbeef", "beefdead", modulus, 0)

	expected = []int{48879, 57005} // beef, dead
	result = key.E().digits[0:len(expected)]
	for i, e := range expected {
		if e != result[i] {
			t.Errorf("RSAKeyPair().E() failed, expected %v, got %v", expected, result)
			break
		}
	}
	...
	for i, e := range expected {
		if e != result[i] {
			t.Errorf("RSAKeyPair().D() failed, expected %v, got %v", expected, result)
			break
		}
	}
	...
	for i, e := range expected {
		if e != result[i] {
			t.Errorf("RSAKeyPair().M() failed, expected %v, got %v", expected, result)
			break
		}
	}
	...
	for i, e := range expected {
		if e != result[i] {
			t.Errorf("RSAKeyPair().ChunkSize() failed, expected %v, got %v", expected, result)
			break
		}
	}
	...
	for i, e := range expected {
		if e != result[i] {
			t.Errorf("RSAKeyPair().Radix() failed, expected %v, got %v", expected, result)
			break
		}
	}
}
...
func TestEncryptedString(t *testing.T) {
	SetMaxDigits(262)
	s := "password"
	key := NewRSAKeyPair(encryptionExponent, decryptionExponent, modulus, 0)
